...
async function main() {
  ...
  await pool.query(`UPDATE users SET diamonds = 2000 WHERE id = $1`, [aProf.userId]);
  A2.send({ type: 'buy_premium_road' });
  const bought = await A2.wait('premium_road_purchased');
  check(bought.profile.premiumRoad === true && bought.profile.diamonds === 1000, `premium purchased, 1000💎 düştü — kalan ${bought.profile.diamonds}`);
  A2.send({ type: 'buy_premium_road' });

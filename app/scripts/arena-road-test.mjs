...
for (const viewport of [380, 560, 700]) {
  const padding = Math.max(0, (viewport - 450) / 2);
  const contentHeight = ARENAS.length * 450 + padding * 2;
  for (let row = 0; row < ARENAS.length; row++) {
    const offset = row * 450;
    assert(offset <= contentHeight - viewport, 'target offset does not clamp');
    if (viewport >= 450) assert.equal(padding + row * 450 + 225 - offset, viewport / 2);
  }
}

...
export function CofSurface({
  variant = 'base', onPress, onPressIn, selectedLabel, padding = Z.card.contentPadding, radius = 'card', minHeight,
  accessibilityLabel, style, children,
}: {
  variant?: CofSurfaceVariant;
  onPress?: () => void;               // verilirse yüzey etkileşimlidir (rol: button)
  // Basma ANINDA çağrılır. Bu bileşen ses/haptik BİLMEZ; çağıran ekranın kendi
  // geri bildirim çağrısını (triggerFeedback) eski kartlarla BİREBİR aynı anda
  // yapabilmesi için vardır — kart tokenlara taşınırken o çağrı kaybolmasın.
  onPressIn?: () => void;
  ...
  const { scale, onPressIn: pressIn, onPressOut } = useCofPress(inert);
  const r = R[radius];
  const body = (
    <Animated.View
      style={[
  ...
  return (
    <Pressable
      ...
      onPressIn={() => { if (!inert) onPressIn?.(); pressIn(); }}
...
export function CofButton({
  label, onPress, onPressIn: onPressInExternal, variant = 'primary', size, icon, disabled = false, locked = false, loading = false, fullWidth = true,
  ...
  // Basma ANINDA çağrılır. Buton ses/haptik BİLMEZ; çağıran ekranın kendi
  // geri bildirim çağrısını eski butonla BİREBİR aynı anda yapabilmesi içindir.
  onPressIn?: () => void;
  ...
  return (
    <Pressable
      onPress={inert ? undefined : onPress}
      onPressIn={() => { if (!inert) onPressInExternal?.(); onPressIn(); }}
      onPressOut={onPressOut}
      disabled={inert}
